...
#'
#' @export
theme_retro <- function(font = "SF Alien Encounters",
                        ...
                        title.size = 15) {
  th <- ggplot2::theme_minimal() # evt. "theme_minimal %+replace%" i stedet for at gemme nyt object inde i funktionen.

  # Text format:
  # This sets the font, size, type and colour of text for the chart's title

  th$plot.title <- ggplot2::element_text(
    family = font,
    size = title.size,
    face = "bold",
  ...
  th$plot.subtitle <- ggplot2::element_text(
    family = font,
    size = subtitle.size,
    face = "bold",
    color = main.text.color,
  ...
  th$axis.title.x <- ggplot2::element_text(
    family = font,
    size = axis.title.size,
    color = main.text.color
  )
  th$axis.title.y <- ggplot2::element_text(
    family = font,
    size = axis.title.size,
    color = main.text.color,
    angle = 90
  ...
  th$strip.text <- ggplot2::element_text(
    colour = main.text.color,
    size = 12,
    hjust = 0,
    family = font
  )
